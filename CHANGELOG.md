...
> 服务端专用发布：无客户端改动（`CLIENT_VERSION` 保持 2026.09.13.4 不变），无 schema 变更。
> 已部署 236（2026-09-18 CST，提交 `f0baaa0`）：单文件改动（`translations.py`），逐文件对上次部署
> 提交 `e22f575` 校验通过，重启后 active、`/health` 200、journal 无 error。
...
> 服务端专用发布：无客户端改动（`CLIENT_VERSION` 保持 2026.09.13.4 不变），无 schema 变更。
> 已部署 236（2026-09-14 17:29 CST），部署前逐文件校验远端与提交 `e7209f8` 逐字一致（LF 归一化
