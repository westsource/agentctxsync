...
- **跨 Agent 同步**：Hermes / opencode / Reasonix / OpenClaw / WorkBuddy 共享同一会话池。每个客户端拉取**整个会话池**（全部 Agent）并推送本地持有的全部会话，A 的会话可被 B 拉取并写入其本地存储；会话跨设备续写时只推送新增消息
...
```bash
# 选择 agent（hermes | opencode | reasonix | openclaw | workbuddy），默认 hermes
export HERMES_SYNC_AGENT=reasonix
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `HERMES_SYNC_AGENT` | `hermes` | 本地存储适配器：`hermes`/`opencode`/`reasonix`/`openclaw`/`workbuddy` |
