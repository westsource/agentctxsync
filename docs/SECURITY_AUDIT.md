...
3. **客户端路径穿越**：`validate_local_id` 在 dsh/reasonix/opencode/openclaw/workbuddy 的 `write_sessions` 中（历史 codex 引擎已移除）均被调用（拒绝含 `/`、`\`、`.`、`..` 的 id）；hermes 走 SQLite 参数化。SECURITY.md 此条声明属实。
