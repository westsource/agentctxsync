...
| WorkBuddy | `~/.workbuddy-ai` (legacy `~/.workbuddy`) `projects/<slug>/*.jsonl` + `workbuddy.db` | bare id (uuid; owner via `agent_type`) | append-only jsonl; reads merge split copies across project dirs; running sessions (lock files) skipped; ms↔s timestamp conversion; sessions written need a WorkBuddy restart to show in its list |
| DeepSeek Harness (DSH) | `<DSH_HOME or ~/.dsh>/sessions/--<cwd-slug>--/<session-<uuid>>/session.jsonl[.zstd]` (v0 event log, one zstd frame per line; `DSH_HOME` overrides) | bare id (`session-<uuid>`; foreign ids via idmap) | append-only v0 event log (contiguous seq); read/write need `zstandard`; workspace domain is bootstrapped by dsh itself from session headers; projection-cache docs (list titles) are folded on write |
