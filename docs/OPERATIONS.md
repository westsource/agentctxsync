...
| A hermes session is visible on the server but never appears locally; the client logs `skipped N session(s) from profiles/agents not present on this machine` (and 2026.09.12.5 clients log `Restored N` while `imported` stays 0) | The row carries `profile_name='default'` — hermes' local `profile_name` column literal, pushed by older clients. The canonical spelling of the default profile is `""`/NULL, so the pull-side router (which has no `default` key) silently drops the row | Client ≥ 2026.09.12.6 drops the literal on push and routes `default` as the default profile, so existing rows land; update the client (the completeness repair alone re-fetches such rows every cycle without filing them) |