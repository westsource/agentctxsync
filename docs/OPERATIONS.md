...
### Project Sync (shared pool)

Projects are a **workspace-level shared pool**: `/api/projects/pull` returns every visible project of the workspace regardless of which agent asks, exactly like sessions. Each client pushes only what its own agent's store holds:

- **Hermes** — per-profile `projects.db` (`<profile-dir>/projects.db`, alongside `state.db`). The client walks every `projects.db` by profile to sync projects across devices:
...
- **WorkBuddy** — the desktop app's project list is the `workspaces` table in `workbuddy.db` (`path` + `last_opened_at` ms), which has no id/slug/name column. The adapter keeps the canonical identity per path in the sidecar `.workbuddy-sync-projects.json` (written from each pull), mints `wb_<sha1(path_key)>` deterministically for a path the server never saw, and on pull adds a workspace row (creating the directory) for every pulled project path. Existing rows are never re-dated and never deleted; a project whose directory cannot exist on this machine (e.g. a missing drive) simply stays server-side.
- **Capability**: an adapter takes part only when it declares `supports_projects = True` (hermes, workbuddy). Every other agent skips the project stage — no local project store to read or write.
- **Root-shaped paths stay out of the pool**: a drive/filesystem root, the user home or one of its ancestors (`D:/`, `C:/Users`, `C:/Users/<name>`, `/home/<name>`) is never pushed or pulled as a project — it is an ancestor of every session, so its card would swallow the whole subtree, and its meaning differs per machine (`C:/Users/rong` vs `C:/Users/x1`). Clients filter it on both sides (`mcp/adapters/base.py::is_root_project_path`); the paths stay local (a WorkBuddy workspace keeps working, hermes' `projects.db` is untouched). Full rationale: ARCHITECTURE "根目录（home / 盘符根）不入共享项目池".
- **Retiring a project card**: there is no Web/API entry for it (`projects.hidden` exists but nothing in the app writes it, and `project_folders` is union-only). Use `scripts/hide-root-projects.py` on the server: it finds root-shaped projects (dry-run by default), then `--apply` sets `hidden = 1` (removes the card from the Web list and from `/api/projects/pull` for every client, reversible with `--undo`). A hidden project cannot be resurrected by a client push — the push update path never writes `hidden`.
- **Web session association**: the project list on the Web workspace page prefix-matches session `cwd` against project folders (case-insensitively) to show the sessions under each project (consistent with Hermes' native `project_for_path` logic; paths are per-machine and the Web shows the union). Sessions in folders that belong to no project are listed but not grouped — and because the match is not exclusive, a project folder nested inside another project's folder makes those sessions show under both cards.
- **Tools**: `project_push` / `project_pull` can be triggered manually; periodic sync (default 300s) also syncs projects along the way.

### Session Last Activity (`last_activity_at`)

`sessions.last_activity_at` is the session's real last-activity time = the newest **visible** message timestamp. The server **derives** it on every `/push` from the messages in that payload (a client-sent value for the same field is overwritten); a message-less metadata-only push passes the client's own value through. Clients only consume it, writing it into whichever local "last updated" column their agent has (Hermes `last_activity_at`, WorkBuddy `updated_at`+`last_activity_at`, omp title-slot `updatedAt`, OpenClaw index `updatedAt`/`lastInteractionAt`/`lastActivityAt`) — the rule lives in `mcp/adapters/base.py::session_last_activity` (canonical value → newest message → `ended_at` → `now`). Rationale: `ended_at` disagrees per agent (2026-09-13: missing in 5 of 40 pulled sessions, equal to the newest message time in only 3), and the Web already orders by `MAX(message.timestamp)`. Rows that predate the column stay NULL until a client re-pushes them; `scripts/backfill-last-activity.py` fills them in one go (`--apply`; dry-run default; idempotent).
...
- **Pause / resume sync (per session)**: the session list (`/web/workspace/{id}`, `/web/all-sessions`) and the session viewer carry a "pause sync" / "resume sync" control; tick several sessions and use the bulk bar to pause/resume them at once. A paused session is **frozen on the server**: `/push` and the Web import write nothing to it (session metadata, messages, `rev`/`field_rev` and `last_synced_at` all stay put), while `/pull` keeps delivering the frozen copy to every device — pause only stops the upload direction. Clients re-send paused sessions every cycle (they record no push fingerprint for them), so content written during the pause lands on the server on the next cycle after you resume; nothing is lost. Paused sessions are marked "Sync paused" in the lists. Exact contract (including the older-client window): docs/ARCHITECTURE.md "暂停/恢复同步".
...
| DSH Desktop boots fine but sync never runs (no `mcp__hermes-sync__*` tools, watermark never moves) | The `@deepseek-ai/dsh-mcp-client` row has no `cwd`; the plugin defaults it to `""`, `spawn()` fails with `ENOENT`, and `failOnStartupError: false` hides the startup failure | Add `cwd: '<EXTRACT_DIR>'` to the row in `~/.dsh/profiles/<profile>/cordis.patch.yml` and restart; set `failOnStartupError: true` temporarily to read startup errors on the desktop's startup page (see SUPPORTED_AGENTS.md) |
| A desktop client stays standby forever after restarting the app (tools work, nothing syncs) | A client left over from the previous app instance still holds `hermes-sync-dsh.lock`; the fresh client loses the startup race and never retries as standby | End the stale `agentctxsync-mcp-client-dsh/mcp/server.py` process (or restart the app after killing it) so the new client wins the startup race |
| One hermes session is stuck at an old snapshot on the server while newer ones never arrive | The client's whole push cycle aborted on a non-JSON-encodable value (Hermes 0.20's `messages.display_identity` BLOB leaked into the canonical message) | Client ≥ 2026.09.12.4 drops binary columns and isolates any remaining offender (`unsendable: [id...]` + a field-level log line); update the client, then restart the host agent |
| A session deleted locally comes back on the next sync (or: deleting one locally does not stick) | By design since client/server 2026.09.12.5: any session the server still holds **visible** is authoritative, so a locally missing row is restored on the next pull (a local delete is not a delete signal — see ARCHITECTURE "本地删除不是删除信号") | To retire a session for every device, soft-delete it server-side (Web workspace → hide / trash): hidden rows are never delivered and never reported missing. Reversible via restore |
| Client ≥ 2026.09.12.5 logs `Restored N session(s) the server still holds` after every sync | Same rule, with the same session missing locally every round — usually a local store that cannot keep the row (read-only/wrong agent store) | Expected only while the session is genuinely absent locally; if it repeats forever, check the adapter's `stores_pulled_sessions` (read-only uploaders set it `False`) |
| A hermes session is visible on the server but never appears locally; the client logs `skipped N session(s) from profiles/agents not present on this machine` (and 2026.09.12.5 clients log `Restored N` while `imported` stays 0) | The row carries `profile_name='default'` — hermes' local `profile_name` column literal, pushed by older clients. The canonical spelling of the default profile is `""`/NULL, so the pull-side router (which has no `default` key) silently drops the row | Client ≥ 2026.09.13.1 drops the literal on push and routes `default` as the default profile, so existing rows land; update the client (the completeness repair alone re-fetches such rows every cycle without filing them) |