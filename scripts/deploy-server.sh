...
echo "API (loopback): http://127.0.0.1:8765/health"
echo "Web UI (loopback): http://127.0.0.1:8765/web/"
echo ""
echo "The app listens on 127.0.0.1 only: front it with a TLS-terminating reverse"
echo "proxy (nginx/caddy) to reach it from another machine."
echo "See docs/server-deployment.md -> '绑定地址与反向代理（必需）'."
echo ""
echo "Default admin: admin / (随机密码，首次启动时打印在服务端日志，登录后强制修改)"
echo "Login at https://<your-domain>/web/login  (via the reverse proxy)"
