...
class PlanLimitsTest(unittest.TestCase):
    def test_known_plan_returns_limits(self):
        ...
        with mock.patch.object(db, "get_conn", return_value=FakeCtx(conn)):
            self.assertEqual(db.plan_limits("free"), (200, ["hermes"]))

    def test_unknown_plan_fails_open(self):
        conn = FakeConn(FakeCursor([]))
        with mock.patch.object(db, "get_conn", return_value=FakeCtx(conn)):
            self.assertEqual(db.plan_limits("pro"), (None, None))

    def test_explicit_conn_reused(self):
        conn = FakeConn(FakeCursor([(None, None)]))
        self.assertEqual(db.plan_limits("unlimited", conn), (None, None))
...
class AuditLogTest(unittest.TestCase):
    def test_quota_rejection_writes_full_row(self):
        cur = FakeCursor([])
        conn = FakeConn(cur)
        sync.log_audit(conn, "quota_rejected", 7, 3, "dev-1",
