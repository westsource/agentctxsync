...
expire after TOKEN_TTL, and issuing a new one revokes previous live tokens of
the same (user, purpose). Verified emails are unique at the DB level (partial
unique index on email_normalized); the transactional verify re-checks the
uniqueness before committing so two accounts racing on the same pending email
cannot both win.
"""
import hashlib
import re
...
# Activation and password-reset links share one window: both are opened from
# a mailbox, so a short TTL mostly produces "please resend" round-trips. The
# copy in the mails and pages is derived from this value (mailer._link_window).
TOKEN_TTL = 3 * 3600
